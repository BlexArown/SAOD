...
int bsversion1(int n, int A[], int X, int *C) {
    int L = 0, R = n - 1;
    *C = 0;
    while (L <= R) {
        int m = (L + R) / 2;
...
int bsversion2(int n, int A[], int X, int *C) {
    int L = 0, R = n - 1;
    *C = 0;
    int m;
    while (L < R) {
...
int main() {
    ...
    printf("Результаты 1 вверсии\n");
    for (int i = 1; i <= 10; i++) {
        int C;
        int n = 100 * i;
        int A[n];
        FillInc(n, A);
        int pos = bsversion1(n, A, X, &C);
        if (pos != -1) {
    ...
    printf("Результаты 2 вверсии\n");
    for (int i = 1; i <= 10; i++) {
        int C;
        int n = 100 * i;
        int A[n];
        FillInc(n, A);
        int pos = bsversion2(n, A, X, &C);
        if (pos != -1) {
            printf("Элемент %d найден в массиве размером %d на позиции %d\n", X, n, pos);
        } else {
            printf("Элемент %d не найден в массиве размером %d\n", X, n);
